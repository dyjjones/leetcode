...
impl Parseable {
    fn get(Parseable(vec): Parseable, group_index: usize, sub_index: usize) -> Option<char> {
        ...
        match vec.get(group_index) {
            Some(&ParseGroup{ch, occurrences, ..}) => {
    ...
    fn get_with_original_index(Parseable(vec): Parseable, original_index: usize) -> Option<char> {
        ...
        for ParseGroup{ch: ch, occurrences: occurrences, ..} in vec {
        ...
        loop {
            ...
            match current_expr_group_opt {
                ...
                None => {
                    ...
                    // if parsed_str
                    if parsed_string_vec.len() - 1 == parseable_index && parsed_string_vec.last().occurrences {
...
mod problem_tests {
    ...
    fn test_is_match_complex2() {
        assert_eq!(is_match("aabccd".to_string(), "a*bc.*.".to_string()), false);
    }
    #[test]
